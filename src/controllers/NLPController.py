...
class NLPController(BaseController):
    ...
    async def index_into_vectordb(self, project: Project, 
        ...
        vectors = self.embedding_client.embed_text(texts, document_type=EmbeddingDocumentTypeEnums.DOCUMENT.value)
        ...
        _ = await self.vectordb_client.instert_many(
           collection_name=collection_name,
           texts = texts,
           vectors = vectors,
           metadatas = metadatas,
    ...
    async def search_vectordb_collection(self, project: Project, text: str, limit: int = 10):
      ...
      query_vector = None
      query_vectors = self.embedding_client.embed_text(text, document_type=EmbeddingDocumentTypeEnums.QUERY.value)
    ...
    async def answer_query(self, project: Project, query: str, limit: int = 10):
       #retrieve relevant chunks from vectordb
       #construct LLM prompt

       response, final_prompt, chat_history = None, None, None

       retrieved_chunks = await self.search_vectordb_collection(project, query, limit)
